...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const canAccessPath = (path: string): boolean => {
    ...
    if (user.role === 'sales_agent') {
      const allowed = ['/dashboard', '/customers', '/orders', '/products', '/out-of-stock', '/whatsapp-conversations', '/whatsapp-simulator', '/notifications', '/shift-handover'];
      return allowed.some(p => normalized.startsWith(p));
    }

    if (user.role === 'support_agent') {
      const allowed = ['/dashboard', '/customers', '/queries', '/orders', '/products', '/out-of-stock', '/whatsapp-conversations', '/whatsapp-simulator', '/notifications', '/shift-handover'];
      return allowed.some(p => normalized.startsWith(p));
    }
