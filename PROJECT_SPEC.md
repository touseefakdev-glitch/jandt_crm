...
### Orders Technical Architecture (Realtime Rebuild)
- **Workflow Extension**: Daily order operations now also track `POD Sent` (after `Dispatched`, via a checkbox in the Orders table) and an `Order Match` adjudication (`SAME`/`DIFFERENT`) with an optional `difference_note` and `invoice_updated` flag. An `Operational Error` path sets `exception_status = ERROR` with an exception note, query ticket link (`error_query_id` → `customer_queries`), and a derived `status = 'error'`.
...
- **Realtime**: `src/services/realtime.ts` (`subscribeOrdersRealtime`) subscribes to Postgres Changes on `daily_order_operations` + `daily_order_operation_history` (published via migration `04_orders_realtime.sql`). Merges arrive into `storagePrime` (no write-through → no feedback loops) with an identical-value no-op guard, and a 30-second polling fallback (`updated_at` watermark) covers missed events. The Orders page shows a sync badge: `Live Sync` / `Auto-Refresh` / `Local Only`.
- **RLS & Auth**: Migration `04_orders_realtime.sql` enables Row Level Security on `route_schedules`, `daily_order_operations`, and `daily_order_operation_history` (dropping the legacy permissive `App anon full access` seed policies) and adds area-scoped `FOR ALL` policies. Identity is resolved by email (`p.email = auth.jwt() ->> 'email'`) so profiles not yet matched to an `auth.users` UUID still get access. **Requirement**: after applying migration 04, Supabase Auth users must exist for the seeded emails (with their password) — the frontend does a best-effort `signInWithPassword` on local login; without an authenticated session the tables become read-only to the app.
- **Backend queries**: `getDailyOrderOperations` auto-derives area, filters schedules by area, and joins pod-sender / updater / error-query details; new methods `updateDailyOrderMatch`, `setDailyOrderException`, `clearDailyOrderException` (status re-derived from workflow booleans), and `getOperationalDatesForRange`. New indexes: `idx_daily_ops_date_route`, `idx_daily_ops_area`, `idx_daily_ops_status`, `idx_daily_ops_updated_at`, `idx_daily_ops_history_op_ts`, `idx_orders_updated_at`.
...
- **Capability model (`src/services/access.ts`)**: A granular, role-and-area derived capability layer is the single frontend source of truth for "what may this user do". No usernames are hard-coded — every decision flows from `role` + `operational_area` + workflow state. Capabilities include `orders:receive`, `orders:createSalesOrder`, `orders:invoice`, `orders:dispatch`, `orders:sendPod`, `orders:updateOrderMatch`, `queries:view`, `queries:manage`, `queries:verify`, `customers:*`, `products:*`, and `admin`. The backend/RLS remains the enforcement layer; the UI is defense-in-depth only.
- **Permission-derived navigation**: The sidebar is built dynamically from capabilities into sections `Work` (My Work, Dashboard), `Operations` (Daily Operations, Customers, Products, Out of Stock, Notifications, Shift Handover), `Queries` (Support Queries — admin/support only), and `Administration` (admin only). Live queue badges appear next to `Daily Operations` (pending today, area-scoped) and `Support Queries` (active open count).
- **My Work center (`/my-work`, index redirect)**: A per-user action-queue page. Cards ("Orders To Receive", "Sales Orders To Generate", "Invoices To Raise", "Orders To Dispatch", "POD To Send", "Order Match Review", "My Open Queries", "Queries Awaiting Verification", "Open Queries") are computed from capabilities; only cards the user may act on are rendered. Counts come from the live local store and stay in sync across views. Each card deep-links to the pre-filtered list (Orders page honors `?status=` / `?quick=different`; Queries page honors `?tab=` / `?status=`).
- **Role-aware dashboard**: The dashboard now surfaces the user's action queues ("My Work — Action Queues") immediately below the welcome banner, in addition to the existing KPIs.
- **Query resolution verification (four-eyes)**: New `verified` query status completes the flow `OPEN → ASSIGNED → IN_PROGRESS → RESOLVED → VERIFIED → CLOSED`. Verification is a second-person check — `canVerifyQuery` blocks a user from verifying a query they resolved themselves ("Fail Verification" returns the ticket to `IN_PROGRESS`). `verified_at` / `verified_by` are recorded on the query and surfaced in the Query Detail resolution record. Applied via migration `05_query_verified_status.sql` (enum `ADD VALUE` + columns + index); `database/schema.sql` updated.
- **Scope note**: The order-level `VERIFIED` (between Sales Order and Invoice) and `AUDITED` (final) stages, the dedicated Verification / Audit centers, and the REPORTS module are the next Phase 2 increments — they require new `daily_order_operations` columns plus center pages and are not yet shipped.
